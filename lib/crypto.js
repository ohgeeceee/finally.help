...
// Used to hand the browser an opaque token whose contents it cannot read or
// forge (e.g. the correct answer to a quiz question). Key = SHA-256(secret).
...
export async function unseal(secret, token) {
  try {
    ...
    const pt = await crypto.subtle.decrypt(
      { name: "AES-GCM", iv: b64urlDecode(ivb) }, key, b64urlDecode(ctb)
    );
